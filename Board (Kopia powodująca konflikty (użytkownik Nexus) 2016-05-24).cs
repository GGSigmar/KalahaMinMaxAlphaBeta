...
namespace Bantumi
{
    public class Board
    {
        ...
        public bool AskForMove(bool player) // ruch gracza
        {
            ...
            return PlayerMove(player, choice); // zwraca czy jeste dodatkowy ruch
        }
        ...
        public int SeekAndDestroy(Board oldBoard, int depth, bool max, int move, int alpha, int beta) // minmax really
        {
        ...
        public int HeadQuarter(int depth) // zwraca choice z najwyzszym scorem
        {
            ...
            for (int i = 1; i < 7; i++)
            {
                // TO DO not looking at empty cells
